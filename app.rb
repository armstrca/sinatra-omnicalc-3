...
# a hash that specifies what model of ChatGPT should be initialized on the OpenAI end and transmits the request_messages hash "silently." 
# Doing it this way because it allows for separating the @current_message variable from the entirety of the @chat_history variable, 
# which ensures that the OpenAI server "remembers" all cookie inputs but only directly responds to @current_message. 
# Without this, each ChatGPT reply would include responses to every single user message in the @chat_history array.
